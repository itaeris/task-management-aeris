  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
      <section className={cn(surface, "rounded-3xl p-6")}>
        <h1 className="font-serif text-3xl">Share & kolaborasi</h1>
        <p className="mt-1 text-sm text-muted">
          Undang tim lewat kode atau tautan. Setiap anggota bisa komentar, unggah file, dan isi daily check.
        </p>
        <div className="mt-5 rounded-2xl bg-gradient-to-r from-sky-500 to-blue-600 px-5 py-4 text-white">
          <p className="text-xs tracking-[0.18em] text-white/70 uppercase">Kode share</p>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            >
              {copied ? <Check size={14} /> : <Link2 size={14} />} {copied ? "Tersalin" : "Salin tautan"}
            </button>
            {role === "owner" ? (
        ...
        {role === "owner" ? (
          <form className="mt-6 grid gap-3" action={(formData) => updateProject(projectId, formData)}>
            ...
            <button className={cn(btnPrimary, "self-start")}>Simpan project</button>
          </form>
        ) : (
          <form className="mt-6" action={leaveProject.bind(null, projectId)}>
            <button className={cn(btnGhost, "text-red-700")}>Keluar dari project</button>
          </form>
        )}
      </section>
      <section className={cn(surface, "rounded-3xl p-6")}>
        <h2 className="font-serif text-2xl">Anggota</h2>
